...
- **Affiliate tool LOCKED = Whop native** affiliate program (already own it; 2% fee only).
...
- [ ] Whop: activate store + go live; add the webhook (`/api/whop/webhook (to be built)`, event
- [ ] Whop: turn on the **affiliate program**, set commission **50%**, grab the affiliate
      signup/portal URL → into the `/affiliates` page.
- [ ] 🚩 **COMPLIANCE GATE (Andrew):** confirm with Whop that paying buyers in **NATG token**
...
3. **Affiliate link = 50% commission** on each referred sale (Whop native).
