...
export default function HeroV3() {
  ...
  useEffect(() => {
    ...
    tl.fromTo(book, { opacity: 0, scale: 0.94, x: 30 }, { opacity: 1, scale: 1, x: 0, duration: 1.2 }, 0.35);
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/subscribe", {
        ...
        body: JSON.stringify({ email, source: "hero-v3" }),
      });
  ...
  return (
    <section ref={sectionRef} className="relative min-h-[100dvh] flex items-center overflow-hidden">
      {/* Subtle earth-cube parallax — 15% opacity behind everything */}
      <div
        ref={earthRef}
        aria-hidden
      ...
      {/* Mesh drift — existing radial gold glow */}
      <div className="absolute inset-0 z-[1] pointer-events-none">
        <div
          className="absolute animate-mesh-drift-a"
          style={{
            width: "900px",
            height: "700px",
            top: "10%",
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 md:px-12 w-full pt-40 md:pt-36 pb-20 md:pb-24">
        <div className="grid grid-cols-1 lg:grid-cols-[1.15fr_0.85fr] gap-12 lg:gap-10 items-center">
          <div ref={contentRef}>
            ...
            >
              <label htmlFor="hero-email-v3" className="sr-only">Email address</label>
              <input
                id="hero-email-v3"
                type="email"
                required
                ...
                className="flex-1 min-w-0 bg-transparent border border-border px-5 py-4 text-sm text-tp placeholder:text-tt/60 focus:outline-none focus:border-gold transition-colors duration-300 rounded-lg disabled:opacity-60"
              />
              <button
                type="submit"
                disabled={locked}
