...
export default function Book3D() {
  ...
  useEffect(() => {
    ...
    function onLeave() {
      ...
      const sheen = sheenRef.current;
      if (sheen) sheen.style.background = "radial-gradient(circle at 50% 50%, rgba(255,255,255,0.10) 0%, transparent 55%)";
    }
  ...
  // Layout structure:
  //   wrapRef  (display: flex — centers the scene, NO perspective here)
  //     sceneRef (perspective: 1600px, transform-style: preserve-3d, fixed dims)
  //       boxRef (transform-style: preserve-3d, rotation)
  //         faces (each with their own translate/rotate)
  //
  // The KEY fix: keep perspective and display:flex on DIFFERENT elements.
  // Flex parents flatten 3D children. So flex goes on the outer wrap, and
  // perspective + preserve-3d go on a non-flex inner scene container.
