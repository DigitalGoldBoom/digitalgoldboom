  ...
  return (
    <section className="relative py-14 md:py-[88px]">
      <div className="max-w-[900px] mx-auto px-6 md:px-12">
        <p className="text-[10px] font-semibold tracking-[0.2em] uppercase text-gold mb-4 text-center">WHAT WAITLIST MEMBERS GET</p>
        <h2 className="text-tp font-extrabold tracking-[-0.03em] leading-[1.3] text-center max-w-[22ch] mx-auto" style={{ fontSize: "clamp(1.8rem, 3.8vw, 3rem)" }}>$238 of value. $0 to join.</h2>
        <div className="mt-14 border border-border rounded-lg overflow-hidden">
          {ITEMS.map((item, i) => (
            <div key={item.name} className={`p-8 md:p-10 flex items-start justify-between gap-6 ${i !== ITEMS.length - 1 ? "border-b border-border" : ""}`}>
          ...
          <div className="bg-[#0E0E14] p-8 md:p-10 flex items-center justify-between gap-6 border-t border-border">
        ...
        <p className="mt-6 text-center text-tt text-xs">Free until the book drops. $39 after. No spam.</p>
      </div>
    </section>
  );
}
