...
/**
 * DepositMedia — MOBILE-FIRST. The centrepiece is a light static JPEG of the deposit
 * that renders from plain HTML on ANY device (loads instantly, no WebGL, can't go
 * black). Only DESKTOP-class devices (fine pointer + wide screen) with motion allowed
 * upgrade to the live, draggable 3D model. So phones always get the reliable still;
 * desktops get the spinning 3D. Same deposit either way — the look never downgrades.
 */
const NOOP = () => () => {};
function motionOK() {
...
function DepositMedia() {
  // live 3D on every device with motion allowed (phones included); reduced-motion -> still
  const use3D = useSyncExternalStore(NOOP, motionOK, () => false);
...
export default function DepositHero3() {
