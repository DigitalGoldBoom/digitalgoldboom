 ...
 *
 * It is not a hand-set switch: it is simply whether a Whop checkout URL exists. While there is
 * none, the site does not pretend otherwise: there is NO fake checkout, no imitation payment form,
 * nothing that collects a card. The product page is real, the price is real, the refund policy is
 * real, and the button leads to the truthful next step (join the list).
 *
 * The day NEXT_PUBLIC_WHOP_CHECKOUT_URL is set in Vercel, this flips to true on its own and every
 * buy button on the site becomes a live checkout. No code change, no second deploy to remember.
 */
export const BOOK_ON_SALE = Boolean(process.env.NEXT_PUBLIC_WHOP_CHECKOUT_URL);
